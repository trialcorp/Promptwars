...
from app.security import sanitize_input, check_rate_limit, _rate_limits
from app.cache import cache_key, cache_get, cache_set, cache_clear, _store
...
class TestIndex:
    ...
    def test_has_textarea(self, client):
        assert b"textarea" in client.get("/").data

    def test_has_language_selector(self, client):
        assert b"lang-select" in client.get("/").data

    def test_has_examples(self, client):
    ...
    def test_has_volunteer_media_branding(self, client):
...
class TestHealth:
    ...
    def test_has_app_name(self, client):
        data = json.loads(client.get("/health").data)
...
class TestReport:
    ...
    def test_error_message_for_empty(self, client):
...
class TestTranslate:
    ...
    def test_returns_translated_key(self, client):
...
class TestFeed:
    ...
    def test_has_reports_key(self, client):
...
class TestZones:
    ...
    def test_has_zones_key(self, client):
...
class TestSecurityHeaders:
    ...
    def test_csp_allows_maps(self, client):
...
class TestCrowdReportModel:
    ...
    def test_input_preview_truncated(self):
        ...
        d = report.to_firestore_dict()
        assert len(d["input_preview"]) == 200
